...
input = PGDB.query_all <<-SQL, as: {Int32, String}
  select id, img_og from upstems
  where img_og <> '' and img_cv = ''
  order by id desc
SQL
...
w_size.times do
  spawn do
    loop do
      sn_id, scover, idx = workers.receive
      bcover = cache_cover(scover)
      PGDB.exec "update upstems set img_cv = $1 where id = $2", bcover, sn_id
      puts "- #{idx}/#{q_size} <#{sn_id}> #{scover.colorize.blue} => [#{bcover.colorize.yellow}]"
    rescue err
