...
def add_pecs_for_puncts
  inputs = AI::MtTerm.db("essence").open_ro do |db|
    query = <<-SQL
    select zstr from terms
    where cpos = 'PU'
    SQL

    db.query_all query, as: String
  end
  ...
  AI::MtTerm.db("essence").open_tx do |db|
    inputs.each do |zstr|
      pecs = AI::MtPecs.parse_punct(zstr).to_s.gsub(" | ", " ")
      db.exec "update terms set pecs = $1 where zstr = $2 and cpos = 'PU'", pecs, zstr
    end
  end
end
...
AI::MtTerm.db("essence").open_ro do |db|
  db_path = AI::MtTerm.db_path("essence").sub(".db3", ".tsv")

  File.open(db_path, "w") do |file|
    AI::MtTerm.get_all(db: db).each do |term|
