...
# struct YS::YscritList
#   def initialize(@crits : Array(Yscrit))
#     @books = Wninfo.preload(crits.map(&.nvinfo_id)).to_h { |x| {x.nvinfo_id, x} }
#     @users = Ysuser.preload(crits.map(&.ysuser_id)).to_h { |x| {x.ysuser_id, x} }
#   end
