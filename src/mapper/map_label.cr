...
class MapLabel::Bulk
  ...
  def load!(file : File = @file) : Void
    ...
    puts "- <map_label> [#{file}] loaded.".colorize(:cyan)
  end

  def save!(file : String = @file) : Void
    File.write(file, self)
    puts "- <map_label> [#{file}] saved, (entries: #{size}).".colorize(:cyan)
  end

  def include?(key : String)
    @data.has_key?(key)
  end
...
module MapLabel
  ...
  def load!(name : String) : Bulk
    CACHE[name] ||= File.new(path(name), preload: true)
  end
