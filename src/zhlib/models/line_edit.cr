...
class ZH::LineEdit
  ...
  def self.insert(input : Hash(String, DB::Any))
    ...
    open_db &.exec("insert into edits(#{fields}) values (#{holder})", args: values)
  end
  ...
  property l_id : Int32, orig : String, edit : String
  ...
  def create!
    ...
    self.class.open_db do |db|
      holder = Array.new(values.size, "?").join(", ")
      db.exec("insert into edits(#{fields.join(", ")}) values (#{holder})", args: values)
    end
  end
end
