...
module MtlV2::MtDict
  ...
  def load(dname : String, df_ptag = PosTag::Unkn, fixed_tag = true)
    DICTS[dname] ||= begin
      vpdict = V2Dict.load_cvmtl(dname)
  ...
  def upsert(dict : String, term : V2Term) : Nil
    load(dict).upsert_term(term)
  end

  def fix_noun!(node : BaseNode) : BaseNode
  ...
  def fix_verb!(node : BaseNode) : BaseNode
  ...
  def fix_adjt!(node : BaseNode) : BaseNode
  ...
  def fix_adverb!(node : BaseNode) : BaseNode
  ...
  def fix_uzhi(node : BaseNode)
    return unless term = self.fix_u_zhi[node.key]?
    node.val = term[0]
    term[1]
  end

  def fix_quanti(node : BaseNode)
    key = node.key
