module MT::TlRule
  ...
  def fold_prepos_inner!(prepos : BaseNode, succ = prepos.succ?, mode = 0) : BaseNode
    ...
    return fold!(prepos, noun, MapTag::PrepForm, dic: 2) unless verb = noun.succ?
    ...
    if verb.pt_dep? && (tail = scan_noun!(verb.succ?))
      ...
      unless verb_2 = find_verb_after_for_prepos(tail, skip_comma: false)
        if tail.succ? { |x| x.boundary? || x.nounish? }
          ...
          return fold!(prepos, noun, MapTag::Vobj, dic: 9)
        end

        if prepos.pre_zai? && (noun = fold_prezai_locale?(prepos, noun, verb, tail))
          return fold!(prepos, noun, MapTag::PrepForm, dic: 3)
        end

        prepos = fold!(prepos, verb.set!(""), MapTag::PrepForm, dic: 5)
        return fold!(prepos, tail, MapTag::Nform, dic: 6, flip: true)
      end
    ...
    head = fold!(prepos, noun, MapTag::PrepForm, dic: 4)
    ...
    #   node = fold!(prev, node, MapTag::SubjVerb, dic: 7) unless mode == 3
    # end
  ...
  def fold_prepos_left(prepos : BaseNode, noun : BaseNode, ude1 : BaseNode, tail : BaseNode)
    ...
    when .nil?, .pro_dems?, .pro_ints?
      left = fold!(prepos, ude1.set!(""), MapTag::DcPhrase, dic: 6)
      fold!(left, tail, MapTag::Nform, dic: 9, flip: true)
    else
      # TODO
    end
  end
