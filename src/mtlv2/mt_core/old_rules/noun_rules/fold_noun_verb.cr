module MT::TlRule
  ...
  def fold_noun_verb!(noun : BaseNode, verb : BaseNode)
    ...
    case tail.key
    when "时候"
      noun = fold!(noun, succ.set!(""), MapTag::DcPhrase, dic: 7)
      return fold!(noun, tail.set!("lúc"), MapTag::Texpr, dic: 9, flip: true)
    end
    ...
    #     tail = fold!(tail, verb_2, MapTag::SubjVerb, dic: 8)
    #     noun = fold!(noun, verb, MapTag::SubjVerb, dic: 7)
    #     return fold!(noun, succ.set!(""), dic: 9, flip: true)
    #   end
    # end

    left = fold!(noun, verb, MapTag::Vform, dic: 4)

    defn = fold!(left, succ.set!(""), MapTag::DcPhrase, dic: 6, flip: true)
