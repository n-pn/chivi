...
module Oldcv::UserDB
  ...
  class_getter emails : Oldcv::LabelMap { Oldcv::LabelMap.load("#{ID_DIR}/emails.txt") }
  class_getter unames : Oldcv::LabelMap { Oldcv::LabelMap.load("#{ID_DIR}/unames.txt") }
  ...
  UBIDS = {} of String => Oldcv::TokenMap

  def ubids(uslug : String)
    UBIDS[uslug] ||= Oldcv::TokenMap.read(File.join(SR_DIR, "#{uslug}.txt"))
  end
