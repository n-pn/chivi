...
require "../mapper/token_map"
require "../mapper/order_map"
require "../models/book_info"

# TODO: change class Oldcv::to module?
require "json"

class Oldcv::BookDB::Query
  struct Opts
  ...
  def filter_genre(genre : String) : Void
    ...
    unless ubids = Oldcv::TokenMap.vi_genres.keys(genre)
      @no_match = true
    else
  ...
  def filter_query(query : String, type = :fuzzy) : Void
    ...
    if type == :fuzzy || type == :title
      if is_hanzi
        @ubids.concat Oldcv::TokenMap.zh_title.search(tokens)
      else
        @ubids.concat Oldcv::TokenMap.vi_title.search(tokens)
        @ubids.concat Oldcv::TokenMap.hv_title.search(tokens)
      end
    end

    if type == :fuzzy || type == :author
      if is_hanzi
        @ubids.concat Oldcv::TokenMap.zh_author.search(tokens)
      else
        @ubids.concat Oldcv::TokenMap.vi_author.search(tokens)
      end
    end
