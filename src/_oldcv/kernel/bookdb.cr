...
module Oldcv::BookDB
  ...
  def find(slug : String)
    return unless ubid = Oldcv::LabelMap.book_slug.fetch(slug)
    BookInfo.get(ubid)
  end
  ...
  def set_hv_title(info : BookInfo, hv_title = "", force = false)
    ...
    Utils.update_token(Oldcv::TokenMap.hv_title, info.ubid, hv_title)
  ...
  def set_vi_title(info : BookInfo, vi_title = "", force = false)
    ...
    if vi_title.empty?
      if title = Oldcv::LabelMap.vi_title.fetch(info.zh_title)
    ...
    Utils.update_token(Oldcv::TokenMap.vi_title, info.ubid, vi_title)
  ...
  def set_vi_author(info : BookInfo, vi_author = "", force = false)
    ...
    if vi_author.empty?
      unless vi_author = Oldcv::LabelMap.vi_author.fetch(info.zh_author)
        vi_author = Utils.hanviet(info.zh_author)
      end
    end

    Utils.update_token(Oldcv::TokenMap.vi_author, info.ubid, vi_author)
    info.vi_author = vi_author
  end

  def check_slug(info : BookInfo, slug : String) : String?
    if ubid = Oldcv::LabelMap.book_slug.fetch(slug)
      return if ubid != info.ubid
    else
      Oldcv::LabelMap.book_slug.upsert!(slug, info.ubid)
      slug
    end
  end
  ...
  def set_genre(info : BookInfo, site : String, genre : String, force = false)
    ...
    Utils.update_token(Oldcv::TokenMap.vi_genres, info.ubid, info.vi_genres)
  end
  ...
  def set_tags(info : BookInfo, site : String, tags : Array(String), force = false)
    ...
    Utils.update_token(Oldcv::TokenMap.vi_tags, info.ubid, info.vi_tags)
  end
