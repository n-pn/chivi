...
class Oldcv::TokenMap
  ...
  def self.read!(file : String) : Oldcv::TokenMap
    new(file, mode: 2)
  end
  ...
  def self.load!(name : String) : Oldcv::TokenMap
  ...
  def self.init(name : String) : Oldcv::TokenMap
    new(path_for(name), mode: 0)
  end

  CACHE = {} of String => Oldcv::TokenMap

  def self.preload!(name : String) : Oldcv::TokenMap
  ...
  class_getter zh_author : Oldcv::TokenMap { preload("indexes/tokens/zh_author") }
  class_getter vi_author : Oldcv::TokenMap { preload("indexes/tokens/vi_author") }

  class_getter zh_title : Oldcv::TokenMap { preload("indexes/tokens/zh_title") }
  class_getter vi_title : Oldcv::TokenMap { preload("indexes/tokens/vi_title") }
  class_getter hv_title : Oldcv::TokenMap { preload("indexes/tokens/hv_title") }

  class_getter vi_genres : Oldcv::TokenMap { preload("indexes/tokens/vi_genres") }
  class_getter vi_tags : Oldcv::TokenMap { preload("indexes/tokens/vi_tags") }
end

# test = Oldcv::TokenMap.new("tmp/token_map.txt")
