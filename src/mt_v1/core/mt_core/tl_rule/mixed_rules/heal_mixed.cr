module M1::TlRule
  def heal_mixed!(node : MtNode, prev = node.prev, succ = node.succ?)
    return node unless node.is_a?(ZvDefn)
  ...
  def heal_vead!(node : ZvDefn, prev : MtNode?, succ : MtNode?) : ZvDefn
    ...
    when .nhanzi?
      return node unless prev.is_a?(ZvDefn)
  ...
  # ameba:disable Metrics/CyclomaticComplexity
  def heal_veno!(node : ZvDefn, prev : MtNode?, succ : MtNode?) : ZvDefn
    # puts [node, prev, succ]
    ...
    when .pre_zai?, .pre_bei?, .vmodal?, .vpro?,
      ...
      return SpDict.fix_verbs.fix!(node)
    when .adjective?
      return SpDict.fix_verbs.fix!(node) unless prev.is_a?(ZvDefn)
  ...
  def heal_ajno!(node : MtNode, prev : MtNode?, succ : MtNode?) : MtNode
    ...
    when .nominal?, .spaces?
      ...
      if succ.is_a?(ZvDefn) && succ.key == "到"
        return SpDict.fix_adjts.fix!(node)
      end
    end
