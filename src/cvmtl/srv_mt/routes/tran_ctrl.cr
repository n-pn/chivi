...
module MT
  class QtranCtrl < BaseCtrl
    ...
    def convert(book = "combine", pack : String? = nil, user : String? = nil,
                rmode = "txt", apply_cap : Bool = true, has_title : Bool = false, use_temp : Bool = false)
      engine = Engine.new(book, pack, user, temp: use_temp)
      to_mtl = rmode == "mtl"
