struct MT::PosTag
  ...
  def self.map_adverb(key : String)
    ADVERB_MAP[key]? || new(:adverb)
  end

  def self.map_suffix(key : String)
    SUFFIX_MAP[key] ||= key[0] == '之' ? new(:suf_zhi) : new(:suf_noun)
  end
  ...
  def self.not_vcompl(key : String)
    NOT_VCOMPL_MAP[key]? || new(:verb)
  end

  def self.not_quanti(key : String)
    NOT_QUANTI_MAP[key]? || new(:noun)
  end

  ###

  TO_NOUN_MAP = load_map("to_noun", MtlPos.flags(Object))
  TO_VERB_MAP = load_map("to_verb", MtlPos.flags(CanBePred))
  TO_ADJT_MAP = load_map("to_adjt", MtlPos.flags(CanBePred))
  TO_ADVB_MAP = load_map("to_advb", MtlPos.flags(CanBePred))

  def self.cast_noun(key : String)
    TO_NOUN_MAP[key]? || new(:noun)
  end

  def self.cast_verb(key : String)
    TO_VERB_MAP[key]? || new(:verb)
  end

  def self.cast_adjt(key : String)
    TO_ADJT_MAP[key]? || new(:adjt)
  end

  def self.cast_advb(key : String)
    TO_ADVB_MAP[key]? || new(:adverb)
  end
end
