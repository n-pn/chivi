module MT::TlRule
  ...
  def fold_prepos_inner!(prepos : MtNode, succ = prepos.succ?, mode = 0) : MtNode
    ...
    return fold!(prepos, noun, PosTag::PrepForm) unless verb = noun.succ?
    ...
    if verb.pt_dep? && (tail = scan_noun!(verb.succ?))
      ...
      unless verb_2 = find_verb_after_for_prepos(tail, skip_comma: false)
        if tail.succ? { |x| x.boundary? || x.nounish? }
          ...
          return fold!(prepos, noun, PosTag::Vobj)
        end

        if prepos.pre_zai? && (noun = fold_prezai_locale?(prepos, noun, verb, tail))
          return fold!(prepos, noun, PosTag::PrepForm)
        end

        prepos = fold!(prepos, verb.set!(""), PosTag::PrepForm)
        return fold!(prepos, tail, PosTag::Nform, flip: true)
      end
    ...
    head = fold!(prepos, noun, PosTag::PrepForm)
    ...
    #   node = fold!(prev, node, PosTag::SubjVerb) unless mode == 3
    # end
  ...
  def fold_prepos_left(prepos : MtNode, noun : MtNode, ude1 : MtNode, tail : MtNode)
    ...
    when .nil?, .pro_dems?, .pro_ints?
      left = fold!(prepos, ude1.set!(""), PosTag::DcPhrase)
      fold!(left, tail, PosTag::Nform, flip: true)
    else
      # TODO
    end
  end
