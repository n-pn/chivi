...
class MT::AiCore
  ...
  private def split_dp_term(term : MtNode, body : MtPair)
    {body.head, body.tail}
  end

  private def split_dp_term(term : MtNode, body : Array(MtNode))
    dt_node = body[0]
    qp_node = init_term(body[1..], epos: :QP, attr: :none)
    {dt_node, qp_node}
  end

  private def split_dp_term(term : MtNode, body : MtDefn | MtNode)
    fchar = term.zstr[0]
    ...
    qp_term = MtNode.new(body: qp_body, zstr: qp_zstr, epos: :QP, from: term.from &+ 1)
