...
class MT::SqPair
  getter hash = {} of String => Hash(String, MtPair)

  def initialize(@dname : String)
  end

  def add!(data : PgPair)
    hash = @hash[data.a_key] ||= {} of String => MtPair
    hash[data.b_key] = MtPair.new(data.a_vstr, data.a_attr, data.b_vstr, data.b_attr)
  end
