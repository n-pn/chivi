...
def test_info(site, bsid, expiry = 6.hours) : Void
  ...
  puts parser.get_infos!.to_pretty_json
  puts parser.get_chaps!.first(10).to_pretty_json
end

def test_text(site, bsid, csid, expiry = 1000.hours)
  parser = TextParser.load(site, bsid, csid, expiry: expiry, frozen: true)

  puts parser.get_title!
  puts parser.get_paras!.first(10)
end
