...
// const ASSET_ARR = build.filter((x) => /entry|nodes/.test(x)) //.concat(files)
// const ASSET_SET = new Set(ASSET_ARR.slice(0, 20))
    ...
    worker.clients
      ...
      .then((clients) => {
        if (clients.length && clients.length > 0) {
          ...
          worker.clients.openWindow('/me/notif').then((window) => (window ? window.focus() : null))
      })
      .catch((e) => {
        console.error(e)
      })
