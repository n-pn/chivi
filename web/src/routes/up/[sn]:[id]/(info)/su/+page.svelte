<script lang="ts">
  ...
  let uform = { ...data.ustem }
  let labels = data.ustem.labels.join(', ')

  let err_text: string

  $: owner = $_user.privi > 3 || uform.owner == $_user.vu_id
  $: action = `/_rd/upstems/${data.ustem.id}`
  ...
  <footer class="delete">
    ...
    <div class="delete-input">
      ...
      <button
        ...
        disabled={!owner || confirm != data.ustem.id}
