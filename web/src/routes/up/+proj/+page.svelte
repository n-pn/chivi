<script lang="ts">
  ...
  const action = '/_up/projs'

  async function submit() {
    const action = form.id ? `/_up/projs/${form.id}` : '/_up/projs'
    const body = { ...form, labels: labels.split(',') }

    try {
      const { id, uslug } = await api_call(action, body, 'POST')
      await goto(`/up/${id}-${uslug}`)
    } catch (ex) {
  ...
  <form {action} method="POST" on:submit|preventDefault={submit}>
    ...
    <div class="form-group labels">
      ...
      <input
        ...
        bind:value={labels} />
      <p class="hints">Tách các nhãn bằng dấu phẩy (,)</p>
      <p class="hints">
        Các nhãn bằng tiếng Trung sẽ được tự đổi sang tiếng Việt
      </p>
    </div>
